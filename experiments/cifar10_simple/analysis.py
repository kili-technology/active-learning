...
if analyze_queries:
    ...
    # config['experiment']['size'] = 10
    dataset, learner = set_up_learner(dataset)(
    ...
    correlation_with_query = np.zeros(
        (10, len(config['experiment']['strategies'])))
    for i in range(10):
    ...
    df_pair = pd.DataFrame(
        correlation_with_query, columns=config['experiment']['strategies'], index=np.arange(10))
    print(df_pair)
